...
namespace Hantek {
    ...
    void Control::run() {
        ...
        while(captureState != LIBUSB_ERROR_NO_DEVICE && !this->terminate) {
            ...
            switch(captureState) {
                case CAPTURE_READY:
                    qDebug("CAPTURE_READY");
                    break;
                case CAPTURE_READY2250:
                    qDebug("CAPTURE_READY2250");
                    break;
                case CAPTURE_READY5200:
                    qDebug("CAPTURE_READY5200");
                ...
                case CAPTURE_SAMPLING:
                    qDebug("CAPTURE_SAMPLING");
                    break;
                default:
                    qDebug("default: captureState = %d",captureState);
                    break;
            }
        }
    ...
    int Control::getCaptureState() {
        ...
        if(errorCode < 0){
            qDebug("Error Code : %d",errorCode);
            return errorCode;
        }
        this->settings.trigger.point = this->calculateTriggerPoint(response.getTriggerPoint());
        qDebug("CaptureState = %d",(int)response.getCaptureState());
    ...
    int Control::getSamples(bool process) {
        ...
        if (this->device->getModel() != MODEL_DSO6022BE) {
            ...
            qDebug("Model = MODEL_DSO6022BE\n");
            qDebug("Trying to comunicate with bulkCommand");
            errorCode = this->device->bulkCommand(this->command[BULK_GETDATA], 1);
            if(errorCode < 0){
                return errorCode;
            }
